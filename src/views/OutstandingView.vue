...
watch(
  ...
  (newVal) => {
    if (newVal == 'All') {
      ...
      displayData.value = rawData.value.filter((item) => item.Classification == newVal)
    }
  },
  { immediate: true }
)
...
const setData = () => {
  ...
  rawData.value.forEach((item) => {
    if (item.Classification != 'Recreation_RestArea' && item.Classification != '')
...
<template>
  <div class="pb-36">
    ...
    <div class="flex flex-wrap justify-between">
      <div v-for="(item, idx) in displayData.slice(0, 30)" :key="item.ID" class="card flex">
        <CardItem :data="item" :layout="cardLayout" :number="`${idx + 1}/${displayData.length}`">
          <template #photo>
            <SlideImage :data="[item.ID]" />
          </template>
          <template #body><span></span></template>
          <template #footer>
            <p class="text-center text-sm font-bold">{{ item.Title }}<br />{{ item.Name }}</p>
          </template>
        </CardItem>
      </div>
    </div>
